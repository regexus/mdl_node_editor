...
RESERVED_WORDS = json.load(open(os.path.join(NODE_TYPES_FOLDER, "reserved_words.info")))
ICON_PATH = os.path.join(MODULE_FOLDER, "icons", "MDL Editor Icon.png")
GRID_PATTERN_PATH = os.path.join(MODULE_FOLDER, "images", "grid_pattern.png")
IMAGE_EXTENSION = ".png"
...
class NodeEditor(QtGui.QGraphicsView):
    ...
    def __init__(self):
        ...
        # Add scene
        self.scene_size = QtCore.QRectF(-3000, -3000, 6000, 6000)
        scene = QtGui.QGraphicsScene(self)
        scene.setSceneRect(self.scene_size)
        self.setScene(scene)
    ...
    def drawBackground(self, painter, rect):
        ...
        def multiply_color_and_image(color, image, factor):

            output_image = QtGui.QImage(image.width(), image.height(), QtGui.QImage.Format_ARGB32)
            pixel_color = QtGui.QColor()
            for x in range(image.width()):
                for y in range(image.height()):

                    pixel_color.setRgba(image.pixel(x, y))
                    pixel_color = QtGui.QColor(color.redF() * ((1-factor) + pixel_color.redF()*factor) * 255,
                                            color.greenF() * ((1-factor) + pixel_color.greenF()*factor) * 255,
                                            color.blueF() * ((1-factor) + pixel_color.blueF()*factor) * 255)
                    output_image.setPixel(x, y, pixel_color.rgba())
        ...
        remapped_scale_factor = (max(scale_factor, FADE_THRESHOLD) - FADE_THRESHOLD)/(1-FADE_THRESHOLD)
        # Set background image
        self.background_pixmap = QtGui.QPixmap()
        self.background_pixmap.convertFromImage(multiply_color_and_image(QtGui.QColor(243, 240, 230), 
                                    QtGui.QImage(GRID_PATTERN_PATH), 
                                    GRID_OPACITY*remapped_scale_factor))

        painter.drawTiledPixmap(self.scene_size, self.background_pixmap)
    ...
    def reserved_words_check(self, published_name):

        if published_name.lower().strip() in RESERVED_WORDS:
            self.show_message("Publish Parameter Error", "You can not use reserved word as a published parameter name.\nPlease try something different.")
            return False
        return True
    ...
    def publish_parameter(self, clicked_node, clicked_slot):
        ...
        if user_text:
            if self.reserved_words_check(user_text):
    ...
    def set_scale_factor(self, scale_factor):
        ...
        m = self.matrix()
        self.setMatrix(QtGui.QMatrix(scale_factor, m.m12(), m.m21(), scale_factor, 0, 0))
    ...
    def wheelEvent(self, event):
        ...
        if event.delta() > 0:
            if current_scale_factor<SCALE_RANGE[1]:
                self.set_scale_factor(current_scale_factor + self.SCALE_STEP)
        else:
            if current_scale_factor>SCALE_RANGE[0]:
                self.set_scale_factor(current_scale_factor - self.SCALE_STEP)

    def show_message(self, window_title, message):

        self.message_dialog = QtGui.QMessageBox(self)
        self.message_dialog.warning(self, window_title, message)
...
class NodeEditorWindow(QtGui.QMainWindow):
    ...
    def add_menu_bar(self):
        ...
        action.triggered.connect(self.node_editor.reset_view)
